                ...
                return (
                  <g key={i} style={{ transformOrigin: `${cx2}px ${cy2}px`, transform: active ? 'scale(1)' : 'scale(0)', transition: 'transform 0.55s cubic-bezier(0.34,1.56,0.64,1)', opacity: active ? 1 : 0 }}>
                    ...
                    {labelLines.map((line, li) => (
                      <text key={li} x={cx2} y={cy2 + (li - (totalLines - 1) / 2) * 14} textAnchor="middle" dominantBaseline="middle" fontSize={b.isTextra ? 17 : 16} fontWeight={b.isTextra ? 900 : 900} fill={b.textFill} fontFamily="var(--font-montserrat), sans-serif">{line}</text>
                    ))}
                    {b.sub.split('\n').map((line, li) => {
                      const baseY = b.isTextra ? b.r + 18 : (totalLines - 1) * 7 + 18;
                      return (
                        <text key={`s${li}`} x={cx2} y={cy2 + baseY + li * 14} textAnchor="middle" fontSize={b.isTextra ? 9 : 8} fontWeight={b.isTextra ? 600 : 500} fill={b.isTextra ? '#fff' : 'rgba(255,255,255,0.3)'} fontFamily="var(--font-montserrat), sans-serif">{line}</text>
                      );
                    })}
                  </g>
                );
